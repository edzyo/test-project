...
RSpec.describe 'Test API' do
  ...
  describe 'GET /categories/:category_id/products' do
    ...
    context 'when category does not exist' do
      ...
      it 'returns a not found message' do
        expect(response.body).to match(/Couldn't find Category with 'id'=0/)
      end
    end
  end
  ...
  describe 'POST /categories/:category_id/products' do
    let(:valid_attributes) { { name: 'Edzyo Product', price: 15 } }
  ...
  describe 'PUT /categories/:category_id/products/:id' do
    let(:valid_attributes) { { name: 'Edzyo' } }
    ...
    context 'when product exists' do
      ...
      it 'updates the product' do
        ...
        expect(updated_product.name).to match(/Edzyo/)
      end
    end
